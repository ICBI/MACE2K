 ...
 */
// HACK-If relQuery, we only want the relationships of the selectedNode

query.generateQueryElements = function (rootNode, selectedNode, links, isConstraintNeeded, useCustomConstraints, isRelQuery) {
    var matchElements = [];
    var whereElements = [];
    ...
    logger.info("Is rel query:", isRelQuery);
...
query.generateNodeRelationQuery = function (targetNode) {
    ...
    var queryElements = query.generateQueryElements(dataModel.getRootNode(), 
            targetNode, linksToRoot, false, false, true);
    var queryMatchElements = queryElements.matchElements,
    ...
    var queryStatement = "MATCH "
    
    if (queryMatchElements.length > 1 ) {
        queryStatement += queryMatchElements[queryMatchElements.length-1]
    } else {
        queryStatement += queryMatchElements.join(", ");
    }
    queryStatement += ((queryWhereElements.length > 0) ? " WHERE " + queryWhereElements.join(" AND ") : "") + " RETURN " + queryReturnElements.join(", ") + " " + queryEndElements.join(" ");
